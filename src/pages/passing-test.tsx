...
export function PassingTestPage() {
  ...
  // Индекс текущего отображаемого задания
  const [currentTaskIndex, setCurrentTaskIndex] = useState(0);

  // Состояние: Хранилище выбранных ответов пользователя.
  // Структура: { [taskId]: { [questionIndex]: number[] } }
  const [selectedAnswers, setSelectedAnswers] = useState<
    Record<number, Record<number, number[]>>
  >({});

  // Хук для работы с роутингом Next.js
  const router = useRouter();

  // ------------------------------------------------------------------
  // ОБРАБОТКА URL-ПАРАМЕТРОВ И ЛОГИКА РЕДИРЕКТА
  // ------------------------------------------------------------------

  // Получаем параметр testIds из URL
  const { testIds } = router.query;

  // useMemo: Парсинг ID патологий из строки testIds в массив чисел.
  const selectedPathologyIds: number[] = useMemo(() => {
    ...
    return ids;
  }, [testIds]); // Пересчитывается только при изменении query параметра testIds

  // useEffect: Редирект на страницу выбора тестов, если не выбраны ID.
  useEffect(() => {
    // Проверка router.isReady гарантирует, что query параметры доступны
  ...
  // Обработчик переключения текущего задания
  ...
  const handleFinishAttempt = () => {
    ...
    // В реальном приложении: отправка данных на сервер и редирект
    // router.push(ROUTES.HOME);
  };

  // Вспомогательная функция для получения выбранных индексов ответов
  const getSelectedFor = (taskId: number, questionIndex: number): number[] =>
    selectedAnswers[taskId]?.[questionIndex] ?? [];

  // Обработчик изменения выбора ответа (чекбокса)
  const toggleAnswer = (
    taskId: number,
    questionIndex: number,
    answerIndex: number,
    typeQuestion: number // 0 - одиночный выбор, 1 - множественный
  ) => {
    setSelectedAnswers((prev) => {
      ...
      if (typeQuestion === 0) {
        ...
        // Множественный выбор: добавить/удалить индекс
