  ...
  return (
    <div className="flex flex-col items-center min-h-screen lg:min-h-[667px]">
      ...
      <div className="flex flex-col justify-between items-center gap-5 flex-1 mb-4">
        <div className="px-5">
          <div className="font-medium text-[20px] font-[#4B4242] mt-7">Содержание атласа</div>
          <UiList className="mt-4 items-start">
            {informationOfPathology.map((item, index) => (
                          <UiListButtonAtlas className="w-full" key={item.id} index={index+1} informationOfPathology={item} />
                        ))}
          </UiList>
        </div>
